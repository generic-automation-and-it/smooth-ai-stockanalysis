...
The caller repository or organization must provide the following GitHub Actions configuration. `secrets: inherit` in `pipeline-code-review-report.yml` resolves against the CALLER repo **and** its organization, so a secret provisioned at the org level satisfies the caller even though it is invisible to `gh secret list` at the repo level. As of 2026-07-25 the required configuration is provisioned and operational: recent `PR Code Review Report` and `PR AI Analyse (Self-Fix)` runs concluded `success`, and review reports were posted to PRs #268 and #269. The authoritative inventory, including scope and missing-credential failure symptoms, lives in [`.github/CI_AGENTS.md`](.github/CI_AGENTS.md). The summary list below is reproduced in this root `AGENTS.md` for discoverability (the root file is more frequently read); the deep table is the single source of truth and is maintained alongside the workflow files it documents.
