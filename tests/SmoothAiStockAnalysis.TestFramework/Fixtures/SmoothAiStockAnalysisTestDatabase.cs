...
/// </remarks>
public sealed class SmoothAiStockAnalysisTestDatabase : IAsyncDisposable
{
    private readonly string _maintenanceConnectionString;

    private SmoothAiStockAnalysisTestDatabase(string connectionString, string databaseName, string maintenanceConnectionString)
    {
    ...
    public static async Task<SmoothAiStockAnalysisTestDatabase> CreateAsync(
        ...
        ITestOutputHelper? output = null)
    {
        ...
        // services.AddDbContext<SmoothAiStockAnalysisDbContext>(opts => opts.UseNpgsql(connectionString));
        // await using var provider = services.BuildServiceProvider();
        // await provider.MigrateSmoothAiStockAnalysisAsync(cancellationToken);

        Log(output, connectionString, $"Database '{databaseName}' ready.");

        return new SmoothAiStockAnalysisTestDatabase(connectionString, databaseName, maintenanceConnectionString);
    }
    ...
    private static void Log(ITestOutputHelper? output, string connectionString, string message)
    {
        try
        {
            ...
            output?.WriteLine($"[SmoothAiStockAnalysisTestDatabase {DateTime.UtcNow:HH:mm:ss.fff}] {b.Host}:{b.Port} — {message}");
        }
        ...
        catch (Exception)
        {
            try
            {
                output?.WriteLine($"[SmoothAiStockAnalysisTestDatabase {DateTime.UtcNow:HH:mm:ss.fff}] {message}");
            }
            catch (InvalidOperationException)
            {
            }
