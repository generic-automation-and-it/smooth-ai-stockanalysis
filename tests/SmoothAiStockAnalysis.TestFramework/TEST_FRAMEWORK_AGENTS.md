...
- **`WebAppFixture<TProgram>`** wraps `WebApplicationFactory<TProgram>` and gives each integration fixture an isolated on-disk SQLite database. Because xunit.v3 compiles test assemblies as executables (each gets its own auto-generated `Program`), an integration test must reference the Host with `Aliases="HostApp"` and close the fixture as `WebAppFixture<HostApp::Program>` to avoid an ambiguous `Program`. Host L2 fixtures inherit committed non-secret configuration such as `DefaultUser:UniqueIdentifier` from the Host `appsettings.json` unless overridden; only the connection string is forced to the isolated file.
- **`SqliteTestDatabase`** is constructed directly (`new SqliteTestDatabase()`) for a fresh temporary file database with pooling disabled (`Guid` path under the OS temp directory), then deletes the database, WAL, shared-memory, and rollback-journal files at disposal. L1 test classes own it and implement `IAsyncDisposable`; xunit.v3 creates a test-class instance per test. `SqliteTestDatabase` constructs a unique path per instance via `Guid.NewGuid():N`, which is per-test in practice because xunit.v3 builds a fresh test-class instance per test; this is what makes parallel-within-level safe (LADR-020). It is the only shared persistence helper; tests must not require a container runtime.
...
| 2026-07-24 | Replaced the `SqliteTestDatabase.Create()` static factory with direct construction, retaining a separate database for each test. | #6 |
| 2026-07-24 | Noted that Host L2 fixtures inherit committed `DefaultUser` placeholder configuration while overriding only the SQLite connection string. | #66, #67 |
| 2026-07-25 | Documented Guid-path isolation as the basis for parallel-within-level CI runs (unit/component). | #83 / WT-10-02 |
