...
| Date | Change | Ref |
|------|--------|-----|
| 2026-07-23 | Local delta from upstream: added an explicit `commit_made_in_step_2` code-block guard in `SKILL.md` Step 4 so literal executors cannot run the trigger check on an unrelated pre-existing `HEAD`, and fail loudly if Step 2 forgot to set the flag. Upstream this fix on next sync. | PR #255 reviews 4767882611, 4767944901 |
