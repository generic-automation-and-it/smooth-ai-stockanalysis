...
- Scaffold HLDs into this repository's visible `docs/hlds/NNN-<slug>/` tree. Keep the script, usage text, skill instructions, and Codex registration aligned with that contract.
- Do not treat a generic downstream template's `.docs/` convention as an in-repository tooling reference. Consumers of that template choose their own documentation layout.
...
| Date | Change | Ref |
|:-----|:-------|:----|
| 2026-07-23 | Corrected the HLD scaffold contract to the visible `docs/hlds/` tree and documented the generic template's intentional `.docs/` exception. | #74 |
