...
- **The `claude-code` platform additionally keeps `--no-skills --no-hooks`.** Its skills/hooks resolve through
  the committed `.claude -> .agents` symlink into `.agents/skills/` (66 tracked files) and
...
| Date | Change | Ref |
| :---- | :---- | :---- |
| 2026-07-30 | Ported from smooth-llm-imposter. Updated wiki path prefix (`.docs/` → `docs/`) and skills file count (81 → 66) to match this repository. | — |
