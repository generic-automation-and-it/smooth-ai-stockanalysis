...
- **Override semantics are nullable-based**: a `null` preference is the explicit "unset" signal that falls through to the application default. Zero, empty string, and `TimeSpan.Zero` are valid business values, never sentinels.
...
- `IApplicationDefaults.GetDefaultDeliveryWindow()` returns a fully-constructed `DeliveryWindow` so the resolver can return either the override window or the default window by reference. The catalogue parses `Cycle:DeliveryWindowStart` / `Cycle:DeliveryWindowEnd` once at composition, not on every resolve.
...
- **L0:** `Application.UnitTest/EffectiveSettingsTests.cs` exercises the pure merge across every typed shape: empty metadata resolves to defaults; full override replaces every value; partial override falls through per field; `DeliveryWindow` override substitutes supplied strings and falls through for the unspecified TZ; malformed `HH:mm` and unknown TZDB zones throw; blank string provider overrides fall through; null decimal and null int overrides fall through.
- **L0:** `Domain.UnitTest/UserMetadataTests.cs` covers v2 metadata creation, all-preferences-null, `WithPreferences` immutability, and `Reconstitute` accepting any positive schema version (so legacy v1 documents read back as v1 with no preferences).
