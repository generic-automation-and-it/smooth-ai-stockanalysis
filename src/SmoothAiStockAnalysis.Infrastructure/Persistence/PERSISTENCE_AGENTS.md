...
- The metadata document carries `"schemaVersion":2` after the F-004 worktask (one nullable property per catalogue key) and `"schemaVersion":1` on a legacy pre-F-004 database. The Infrastructure document translates both to the Domain `UserMetadata` and preserves unknown forward-compatible members through `[JsonExtensionData]`. Persisted metadata updates merge into the tracked document and reject schema-version regression (LADR-015). No EF migration accompanies the schema bump: the column shape is unchanged, only the document payload evolved.
...
- **L1:** `Infrastructure.ComponentTest/UserMetadataPreferencesDocumentTests.cs` proves the production `UserMetadataDocument` with the F-004 v2 typed preferences: round-trip of representative preferences and schema version, retention of unknown forward-compatible fields across a read-modify-write cycle, translation through `ToDomain` (and its mirror `WithPreferences`), legacy v1 on-disk payload reads back as v1 with all preferences unset, and `ApplyDomainState` rejects schema-version regression.
