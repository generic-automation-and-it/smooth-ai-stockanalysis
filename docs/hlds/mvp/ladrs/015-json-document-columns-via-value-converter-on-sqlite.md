...
LADR-010 requires a versioned user-metadata document from the first release, and NFR-048 requires that document to carry an explicit version marker. NFR-045 resolves every tunable value as *user preference first, application default second*, so the metadata document is read and written **as a whole per user** — it is never queried field-by-field.
