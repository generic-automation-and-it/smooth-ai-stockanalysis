...
| L1 | Component | `Application.ComponentTest`, `Infrastructure.ComponentTest` | One layer end-to-end; EF in-memory (Application) or isolated SQLite files (Infrastructure) | Live providers, shared DB servers | No container runtime today | `DOCKER_HOST=unix:///nonexistent bash .github/actions/test-with-coverage/run-level.sh component` |
